...
description: 'Use under the supervisor to define and enforce input, tool-call, and output guardrails plus stop conditions for an approved agent in app-configs'
...
You define the runtime controls that wrap an approved agent in `app-configs`. A valid, well-matched spec can still misbehave at execution time; guardrails are the last layer before and during a run.
