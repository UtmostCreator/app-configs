...
- Avoid by default: `repo-docs/archive/, repo-docs/migration-*.md`
...
- Ask for approval before making: `secrets, destructive changes, NixOS system layer (sys-setup), install/apply scripts, uninstall actions`
...
- Prioritize review around: `correctness, secrets leakage, idempotency of install/apply, host-profile compatibility, chezmoi/nix template drift`
- Use `bash ops/validate-config.sh` as the main verification command unless the task needs a narrower command first.
