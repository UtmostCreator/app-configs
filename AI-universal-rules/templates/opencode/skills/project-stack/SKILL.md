---
name: project-stack
description: Repository-specific conventions for architecture, verification, and review
compatibility: opencode
---

## What I Do

I provide project-specific context for `<PROJECT_NAME>`.

## When To Use Me

- before architecture decisions
- before implementation in unfamiliar areas
- before review or refactor
- when choosing verification

## Project Shape

- Project type: `<PROJECT_TYPE>`
- Summary: `<PROJECT_SUMMARY>`
- Primary language: `<PRIMARY_LANGUAGE>`
- Primary runtime: `<PRIMARY_RUNTIME>`
- Active paths: `<ACTIVE_PATHS>`
- Inactive paths: `<INACTIVE_PATHS>`
- Targets: `<TARGET_PLATFORMS>`

## Architecture Notes

- Primary entrypoints: `<PRIMARY_ENTRYPOINTS>`
- Notes: `<ARCHITECTURE_NOTES>`
- Risk areas: `<RISK_AREAS>`

## Verification Expectations

- Main verification command: `<PRIMARY_VERIFY_COMMAND>`
- Main build command: `<PRIMARY_BUILD_COMMAND>`
- Main test command: `<PRIMARY_TEST_COMMAND>`

## Review Priorities

- `<REVIEW_PRIORITIES>`

## Approval Boundaries

- `<APPROVAL_REQUIRED_CHANGES>`
