...
1. `scripts/ai/ai-search.sh` as the unified search entrypoint for `text`, `files`, `struct`, `tracked`, and `all` modes.
2. `scripts/ai/rg-code.sh` (`rg`) for broad search.
3. `scripts/ai/fd-files.sh` (`fd`) for file discovery.
4. `fzf` for interactive narrowing.
5. `scripts/ai/preview-file.sh` (`bat`) for previews.
6. `git grep` for tracked-tree only search.
7. `scripts/ai/git-forensics.sh` for `git log -S/-G/-L` and `git blame -L`.
8. `scripts/ai/gh-pr-context.sh` plus `jq` or GitHub MCP for remote context, optional diff/check/review capture, and PR-scoped context packing.
...
1. `scripts/ai/pack-context.sh auto .` to auto-select the best installed packer.
2. `scripts/ai/pack-context.sh repomix ...` for full-repo context packaging.
3. `scripts/ai/pack-context.sh files-to-prompt ...` for focused file-list packaging.
4. `scripts/ai/pack-context.sh code2prompt ...` for template-driven provider-specific context files.
5. `scripts/ai/repomix-context-tree.sh` when you want a root index plus child indexes and only want to load the smallest relevant bundle first.
6. `tokei` before packaging when you need language/scope metrics in the prompt preface.
7. `scripts/ai/repomix-scc-router.sh` only when you specifically want legacy ranked per-folder bundles instead of the tree-context workflow.
8. `scripts/ai/ai-diff-context.sh` when you need PR-local, unstaged, recent, or touched-file context instead of a broad repo pack.
9. `scripts/ai/run-repomix-context.sh` when you want one command that validates dependencies, generates the tree outputs, and prints wiring guidance.
...
1. `scripts/ai/watch-loop.sh "<verify-command>" "<ext-list>"` for edit -> run -> observe loops.
...
Wrapper scripts in `scripts/ai/` are classified by the three-tier command risk taxonomy in `docs/ai/command-risk-taxonomy.md`. Tier is determined by invocation shape, not script name.
...
1. Use `scripts/ai/ai-edit.sh` for broad repository edits.
