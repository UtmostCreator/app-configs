#!/usr/bin/env bash
# Fresh Ubuntu container dry-run of ops/bootstrap.sh.
#
...
#
# Usage:
#   bash ops/test-bootstrap-docker.sh             # dry-run
#   bash ops/test-bootstrap-docker.sh --apply     # full --yes inside container
#   IMAGE=debian:12 bash ops/test-bootstrap-docker.sh
...
docker run --rm \
  ...
  bash -lc '
    ...
    su - tester -lc "
      ...
      bash ops/bootstrap.sh ${BOOTSTRAP_FLAG}
    "
  '
