...
#   9. light cleanup             (delegates to ops/cleanup.sh, safe mode)
#
...
#
# Usage:
#   bash ops/update-all.sh              # report only
#   bash ops/update-all.sh --apply      # update, with one confirmation
#   bash ops/update-all.sh --apply --yes   # fully unattended
#   NO_CLEANUP=1 bash ops/update-all.sh --apply   # skip step 9
#
...
if [[ "$MODE" == "apply" && -x "$REPO_ROOT/ops/snapshot-home.sh" ]]; then
...
if [[ -x "$REPO_ROOT/ops/syncthing-obsidian-stignore.sh" ]]; then
  if [[ "$MODE" == "apply" ]]; then
    bash "$REPO_ROOT/ops/syncthing-obsidian-stignore.sh" --apply \
      || warn "syncthing-obsidian-stignore returned non-zero"
  else
    log "would run: bash ops/syncthing-obsidian-stignore.sh --apply"
  fi
else
  warn "ops/syncthing-obsidian-stignore.sh not found (skip)"
fi
...
elif [[ -x "$REPO_ROOT/ops/cleanup.sh" ]]; then
  if [[ "$MODE" == "apply" ]]; then
    bash "$REPO_ROOT/ops/cleanup.sh" --apply --yes || warn "cleanup returned non-zero"
  else
    log "would run: bash ops/cleanup.sh --apply --yes"
  fi
else
