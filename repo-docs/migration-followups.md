...
| --- | --- | --- | --- |
| 2026-09-05 | 149 surviving files reference the removed `tools/ai/`, `scripts/ai/`, `schemas/ai/`, or `docs/ai/` surfaces | The requested deletion was intentionally limited to the explicitly listed legacy AI-kit paths. Removing or rewriting generated/runtime adapters such as `.github/workflows/validate-ai-surface.yml`, `.github/hooks/tool-policy.json`, `.opencode/**`, `opencode.jsonc`, `AGENTS.md`, and `llms.txt` would materially widen the destructive scope, and no tracked RestSift integration was found to use as a proven replacement contract. | Confirm the RestSift spelling, tracked integration path, and whether the obsolete `.github`, `.opencode`, `.ai`, policy, test, and root-document adapters should be deleted or rewritten; then remove all dead entrypoints and add a focused reference check. |
...
| 2026-07-04 | `nix flake check ./nix` FAILS on `checks.x86_64-linux.formatting` | The gate now runs (see Resolved: the old Determinate-installer/sudo blocker no longer applies on this NixOS host). Ran `mise run nix:check`; `treefmt` reports `nix/modules/home/gui.nix` is not formatted per the repo's canonical `nixfmt-rfc-style` — the `androidStudioQuail` `let` binding in that file is currently dirty/unformatted working-tree content from an unrelated in-progress slice (niri/hyprland exploration), not something this doc-correction slice touched or should fix. | Whoever lands the `gui.nix` changes should run `mise run nix:fmt` (or `nix fmt ./nix`) before committing, then re-run `mise run nix:check` to confirm exit 0. |
...
|------|--------------|-------------------|
| `home-manager switch --flake ./nix#wsl --dry-run` | Needs Nix and `home-manager` first (Nix is now installed on the primary NixOS host, but this specific dry-run target has not been re-attempted) | Run `nix profile install nixpkgs#home-manager` on the target host, then the dry-run command. |
| `darwin-rebuild check --flake ./nix#macos` | macOS host required | Validate on a Mac before any production macOS use. |
| `bash ops/test-bootstrap-docker.sh` | docker not installed on this WSL host | Install Docker, then re-run before legacy-cleanup of `backup-sanitized/` is merged. |
...
|------|----------|-------------------|
| `docs/ai/script-registry.json` references `ops/ai/install-mandatory-tools.sh` which does not exist on disk | Generated mirror (per `docs/ai/script-registry.md`, source of truth is `tools/ai/install/script-registry.php`). Hand-editing a generated artifact is out of scope; the generator should be re-run. | Run the registry generator (`php tools/ai/...`) in a dedicated slice and commit the regenerated `script-registry.json`. |
| Dual-boot activation on the live host is NOT done | This slice only added the opt-in `nix/modules/nixos/dual-boot.nix` module + read-only `ops/detect-os-disks.sh`. No `/etc/nixos` edit, no `nixos-rebuild`, no `efibootmgr` boot-order change (boot/auth-sensitive, needs approval). The full step-by-step is now documented in `repo-docs/nixos-rebuild.md` ("Dual-boot with Windows"). | Separate approved slice: import module in `/etc/nixos`, run `ops/detect-os-disks.sh`, discover `windowsDeviceHandle` via EDK2 `map -c`, `nixos-rebuild switch`, then `efibootmgr -o` to make NixOS default. |
...
still supported; Windows-native docs are out-of-scope-but-kept). One genuine
item remains, recorded here (no edits made to code — it is approval-gated/
judgment). A second item, `.husky/` hook cleanup, was already completed — see
the 2026-06-02 "Gated candidates — RESOLVED" entry below (`670fee3`); this
section's stale duplicate "open" row, which contradicted that resolution, has
been removed.

| Item | Why open | Unblock condition |
|------|----------|-------------------|
| `repo-docs/install-dev-tools.sh` content drift | Legacy macOS Homebrew reference. Still REFERENCED (parsed by `ops/generate-package-matrix.sh` for the package list; presence-checked by `ops/doctor.sh`), so not an orphan. But it lists `direnv` (repo dropped direnv per Phase 3) and only wires zsh, while the repo is now fish-first. No current doc tells users to run it. | Judgment call in a dedicated slice: drop `direnv` from `FORMULAE`/wiring and add fish wiring (or annotate as macOS/zsh-legacy reference), then re-verify `ops/doctor.sh` + `ops/generate-package-matrix.sh` output still match. |
...
| B-root | README, CONTRIBUTING, SECURITY (3) | **Kept** — all path refs resolve (README "MISS" hits were bare basenames that exist under `ops/`). | `rg` path-existence + `fd` basename resolution |
...
the moved docs and MUST be rewritten in the move slice: `ops/doctor.sh`,
`ops/validate-config.sh`, `ops/check-source-of-truth.sh`,
`ops/generate-package-matrix.sh`, `ops/install.sh`,
`ops/readiness.sh`. Full 36-file rewrite set is enumerated in plan §3.3.
...
|------|-------|-------------------|
| 2026-07-04 | **`nix flake check ./nix` gate run for the first time** — previously deferred; the cited blocker (Determinate Systems installer needing interactive `sudo`) does not apply, because this NixOS host provides system Nix at `/run/current-system/sw/bin/nix` (confirmed via `command -v nix`), not the Determinate installer. | Ran `mise run nix:check` (`nix flake check ./nix`) — read-only evaluation/build, no system activation. Result: **FAIL, exit 1** — `checks.x86_64-linux.formatting` fails because `nix/modules/home/gui.nix` (unrelated, currently-dirty niri/hyprland exploration work) is not `nixfmt-rfc-style`-formatted. The sudo/Determinate blocker itself no longer applies; see the new "Open" row above for the actual formatting failure to fix before this gate is fully green. |
...
| 2026-05-26 | **`.opencode/agents/implementer.md` + `.opencode/opencode.json` permission widening** — this doc's "Open" section previously said the widening was uncommitted per user instruction "do not commit this files for now." | Already landed in `b10ae88` ("relax deny->ask across permission gates"), converting `deny` entries to `ask`/`allow` in both files. The stale "Open" row described a state that no longer existed and has been removed. **Separately noted:** the dirty-worktree diff still present on `.opencode/opencode.json` as of 2026-07-04 is an UNRELATED, correct fix — a 17-line `ops/ai/*` -> `scripts/ai/*` permission-key rename, because `ops/ai/` does not exist on disk (`scripts/ai/` does). Committed on its own. |
...
| 2026-05-24 | `ops/doctor.sh` not executable | `72fce1d` (Phase 7 — `chmod +x ops/doctor.sh`) |
| 2026-05-24 | `ops/doctor.sh` required deleted `configs/shell/.zshrc` | `0e614dc` (`fix(doctor,policy): align doctor checks ...`) |
| 2026-05-24 | `ops/doctor.sh` carried `add_winget_paths()` Windows code | `0e614dc` (function removed) |
| 2026-05-24 | `ops/unix/ssh-agent-setup.sh` `SNIPPET_DIR` pointed at deleted `configs/shell/ssh-agent/` (would break bootstrap step 8) | `a05eb30` (staleness sweep) |
...
| 2026-05-24 | `CONTRIBUTING.md` referenced non-existent `ops/repo-health-check.sh` and `just *` targets | `a05eb30` (staleness sweep, partial — see C3 above for the corresponding hook bug) |
...
| 2026-05-24 | **C3** — `ops/hooks/pre-commit.sh` called non-existent `ops/repo-health-check.sh` | Delegated the staged-files health check to `ops/doctor.sh` + `ops/validate-config.sh`; missing-script branch downgraded to a warning so a partial clone still commits. |
| 2026-05-24 | **C4** — `ops/doctor.sh` listed `php` as required, but `mise.toml` no longer pins a runtime | `php` moved to optional binaries; AI workflow validators gated on `command -v php` and report a `[WARN]` block instead of failing when absent. Fresh Ubuntu hosts no longer abort at bootstrap step 9. |
