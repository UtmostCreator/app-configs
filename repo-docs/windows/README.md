...
| [ops/Enable-SshAgentService.ps1](ops/Enable-SshAgentService.ps1) | Admin one-shot: enables the Windows OpenSSH `ssh-agent` service so it starts automatically and is reachable from every PowerShell session. |
| [ops/Install-SshAgentProfileSnippet.ps1](ops/Install-SshAgentProfileSnippet.ps1) | Non-admin one-shot: installs the 8-hour re-prompt block into your PowerShell profile. Idempotent. Supports `-Remove` and `-IncludeGitSshOverride`. |
| [ops/Setup-SshAgent.ps1](ops/Setup-SshAgent.ps1) | Per-session fallback for when admin is unavailable. Starts Git's bundled `ssh-agent` and loads the key with `ssh-add -t 28800`. No admin, no persistent state. |

## When to use what

- **First time on this PC:** open `QUICKSTART.md` and follow steps 1-3.
- **Need one-off `git fetch`/`git push` from a borrowed shell:** run
  `pwsh -File repo-docs/windows/ops/Setup-SshAgent.ps1` and use git in the
  same window.
