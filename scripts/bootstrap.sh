...
#   6. mise install
#   7. lefthook install
#   8. scripts/unix/ssh-agent-setup.sh         (skipped on WSL)
#   9. scripts/doctor.sh                       (must pass)
#
# Usage:
#   bash scripts/bootstrap.sh                  # dry-run, default
#   bash scripts/bootstrap.sh --dry-run
#   bash scripts/bootstrap.sh --yes            # actually mutate
#   HOST_PROFILE=wsl bash scripts/bootstrap.sh --dry-run
#   HOST_PROFILE_DEFAULT=linux-cli bash scripts/bootstrap.sh
#   CI=true bash scripts/bootstrap.sh --yes    # non-interactive
#
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  if [[ -x "$REPO_ROOT/scripts/snapshot-home.sh" ]]; then
...
if [[ "$HOST_PROFILE" != "wsl" ]] \
   && [[ -x "$REPO_ROOT/scripts/unix/ssh-agent-setup.sh" ]]; then
  if [[ "$MODE" == "dry-run" ]]; then
    log "would: bash scripts/unix/ssh-agent-setup.sh"
  else
    bash "$REPO_ROOT/scripts/unix/ssh-agent-setup.sh" || log "(ssh-agent-setup returned non-zero; review)"
  fi
fi
...
if [[ -f "$REPO_ROOT/scripts/doctor.sh" ]]; then
  ...
  log "doctor: scripts/doctor.sh missing (skip)"
fi
