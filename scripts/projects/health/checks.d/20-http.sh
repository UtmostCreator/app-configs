...
for id in $(proj_health_for_each_project http); do
  ...
  if proj_health_http_ok "$url" "$TRIES" "$SLEEP"; then
    ...
    proj_emit fail "$id" "no 2xx from $url" \
      "bash scripts/projects/repo-checks.sh ${id}; cd \$(yq -r '.projects.${id}.dir' ~/.config/projects/config.yaml) && \$(yq -r '.projects.${id}.service.start_cmd' ~/.config/projects/config.yaml)"
  fi
done
