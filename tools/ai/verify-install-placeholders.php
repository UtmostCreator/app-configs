...
$scanRoots = ['AGENTS.md', 'CLAUDE.md', 'README.md', 'docs/ai', '.github', '.opencode'];
...
if ($unresolvedRequired === []) {
    ...
    if ($unresolvedOptional !== []) {
        ...
        foreach (array_slice($unresolvedOptional, 0, 5) as $finding) {
...
/** @param array<int, string> $requiredTokens @param array<int, array<string, mixed>> $findings */
function scanFile(string $absolutePath, string $target, array $requiredTokens, array &$findings): void
{
    ...
    foreach ($matches[0] as $hit) {
        ...
        $findings[] = [
            ...
            'file' => $relative,
            'line' => $line,
            'required' => in_array($token, $requiredTokens, true),
        ];
    }
}
